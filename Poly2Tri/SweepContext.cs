...
namespace Poly2Tri
{
    internal class SweepContext
    {
        ...
        public void InitTriangulation()
        {
            ...
            for (int i = 0; i < _points.Count; i++)
            {
                TriPoint pt = _points[i];
                Console.WriteLine("InitTriangulation _points i:" + i + " pt.x:" + pt.X + " y:" + pt.Y);
            }
        }

        private void InitEdges(int first, int last)
        {
            Console.WriteLine("InitEdges first:" + first + " last:" + last);
            for (int i = first; i <= last; i++)
            {
                int j = i < last ? i + 1 : first;
                _edge_list.Add(new Edge(_points[i], _points[j]));

                TriPoint pti = _points[i];
                TriPoint ptj = _points[j];
                Console.WriteLine("InitEdges _points i:" + i + " pti:" + pti.ToString() + " j:" + j + " ptj:" + ptj.ToString());
            }
        }
        ...
        public void MapTriangleToNodes(Triangle t)
        {
            for (int i = 0; i < 3; i++)
            {
                if (t.GetNeighbor(i) == null)
                {
                    ...
                    {
                        n.Triangle = t;
                        Console.WriteLine("SweepContext MapTriangleToNodes node:"+ n.ToString() + " triangle:" +t.ToString()); 
                    }
                }
            }
        }
        ...
        public void MeshClean(Triangle triangle)
        {
            ...
            Console.WriteLine("SweepContext MeshClean add triangle:" +triangle.ToString()); 
